...
class Fridge:
    def __init__(self):
        self.contents = []
        with open("product.json", "r", encoding="utf-8") as  product_f:
            contents = json.load(product_f)
        if contents and len(contents) > 0:
            for product_dict in contents:
                self.contents.append(Product(product_dict['name'], product_dict['quantity']))

    def save(self):
        with open("product.json", "w", encoding="utf-8") as product_f:
            contents = []
